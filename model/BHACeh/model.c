...
//cmf to do include yosuke heating models (kawazura:=turbulent heating) and (rowan:=reconnection heating) 
...
double thetae_func(double uu, double rho, double B, double kel)
{
  ...
  if (with_electrons == 0) {
    ...
    thetae = (game-1.)*(MP/ME)/(trat)*uu/rho;
    //if (thetae>100)
    //fprintf(stderr,"thetae %f %f %f %f %f\n",thetae,B,b2,uu,rho);
  }
  //return thetae;
        ...
        double Ucon[NDIM], double Bcon[NDIM])
{
  ...
  //cmf ask George why i<9  I guess for iharm horizon is at i=9?
  //compute location of horizon for BHAC data in log. Kerr-Schild coordinates
  horizoncut=(log(Rh)-log(Rin))/dx[1]+ 1.0;
  //fprintf(stderr,"thetae %f %f %f %f\n",Rin,Rh,dx[1],horizoncut);
  ...
  *Thetae = thetae_func(p[UU][i][j][k], p[KRHO][i][j][k], (*B)/B_unit, p[KEL][i][j][k]);
  ...
  //if(sig > 1.) || i < 20 || j<10 || j>182) {
  //  *Thetae = 0.0;
  //} || i<9
  if(sig > 1 || i<horizoncut) {
    *Thetae = SMALL;
  }
...
void init_data(int argc, char *argv[], Params *params)
{
  ...
  //cmf get data from BHAC postrad data file
  //modified from Davelaar kmonty
  ...
  //cmf all bhac runs use BHAC_MKS --> needs to be modified !!
  METRIC_BHACMKS=1;
  METRIC_eKS =0;
  METRIC_MKS3 = 0;
  ...
  //cut out 5 deg in theta
  //stopx[2]=3.054326191;
  //startx[2]=0.0872664626;
  ...
  fread(&buffer2, sizeof(double), 1, fp);
  gam = (double)buffer2;
  //this should be gamma_e for BHAC gamma_e=gamma_adi
  //---> need to be fixed
  fread(&buffer2, sizeof(double), 1, fp);
  gam = (double)buffer2;
  ...
  fprintf(stderr, "\nHEADER\n");
  fprintf(stderr, "reading BHAC postrad file\n");
  ...
  fprintf(stderr, "metric %d code %d Ndim %d prec %d\n",metric_type,code_type,Ndim,prec);
  fprintf(stderr, "Sim range x1, x2 x3:  %e %e %e %e %e %e\n", startx[1], stopx[1],startx[2], stopx[2], startx[3],stopx[3]);


  
  //cmf check what nprims is in iharm dumps
  //for bhac postrad with electron heating nprimBHAC=9 --> move this to mode.h
  nprimBHAC=9;
  ...
  fprintf(stderr, "Rh,Rmax,Rin,Rout %f %f %f %f\n", Rh,Rmax,Rin,Rout);

  //keep grmonty strucutre
  ...
  //fseek(fp, -nprimBHAC * N1 * N2 * N3 * sizebit, SEEK_END);
  fseek(fp, 256,SEEK_SET);
  int count = 0;
  for (int k = 0; k < N3; k++) {
    for (int j = 0; j < N2; j++) {
      for (int i = 0; i < N1; i++) {
        for (int l = 0; l < nprimBHAC; l++) {
          if (prec == 0) {
  ...
  //cmf some test values 
  //fprintf(stderr,"p-values %f %f\n", p[4][1][39][89]/(gam-1),gam);

  //cmf need to figure out why this is needed -->why?
  //there is no such think in BHAC to iharm converter--> aks Jordy or Oliver?
  for (int k = 0; k < nprimBHAC; k++) {
    for (int j = 0; j < N2; j++) {
      for (int i = 0; i < N1; i++) {
        double p0 = p[k][i][j][0];
...
void report_spectrum(int N_superph_made, Params *params)
{
  ...
  //cmf BHAC postrad format does not have fluid_header --> build from scratch
  h5io_add_group(fid, "/fluid_header");
 
  h5io_add_data_dbl(fid, "/fluid_header/gam",gam);
  h5io_add_group(fid, "/fluid_header/geom");

  h5io_add_group(fid, "/fluid_header/geom/bhac_mks");
  h5io_add_data_dbl(fid, "/fluid_header/geom/bhac_mks/a", a);
  h5io_add_data_dbl(fid, "/fluid_header/geom/bhac_mks/hslope", hslope);
  h5io_add_data_dbl(fid, "/fluid_header/geom/bhac_mks/r_in", exp(startx[1]));
  h5io_add_data_dbl(fid, "/fluid_header/geom/bhac_mks/r_out", exp(stopx[1]));

  h5io_add_data_dbl(fid, "/fluid_header/geom/dx1", dx[1]);
  h5io_add_data_dbl(fid, "/fluid_header/geom/dx2", dx[2]);
  h5io_add_data_dbl(fid, "/fluid_header/geom/dx3", dx[3]);
  h5io_add_data_dbl(fid, "/fluid_header/geom/n_dim", 4);
  h5io_add_data_str(fid, "/fluid_header/geom/has_electrons", "false");
  h5io_add_data_str(fid, "/fluid_header/geom/metric", "BHAC_MKS");
  h5io_add_data_dbl(fid, "/fluid_header/geom/n1", N1);
  h5io_add_data_dbl(fid, "/fluid_header/geom/n2", N2);
  h5io_add_data_dbl(fid, "/fluid_header/geom/n3", N3);
  h5io_add_data_dbl(fid, "/fluid_header/geom/n_prim", 8);
  h5io_add_data_dbl(fid, "/fluid_header/geom/n_prime_passive", 0);

  h5io_add_data_str(fid, "/fluid_header/geom/prim_names", "RHO_UU_U1_U2_U3_B1_B2_B3"); 
   
  fprintf(stderr, "h5 fluid header ok\n");
